...
import { exportUsers } from "./userExportUtils";
  ...
  const fetchUsers = async () => {
    ...
    const [
      ...
      supabase
        .from("users")
        .select(
          "id, name, email, role, is_active, location, phone, profilePhoto, skinTone, waistSize, bustSize, hipSize, size, height, weight, coin_balance, last_login, created_at, last_sign_in_at"
        )
    ...
    const vendorMap = new Map<string, { business_name: string | null; is_verified: boolean | null }>();
    (vendorsData || []).forEach((vendor: any) => {
      vendorMap.set(vendor.user_id, {
    ...
    (ordersData || []).forEach((order: any) => {
      const entry = orderStats.get(order.user_id) || { count: 0, total: 0 };
      entry.count += 1;
      entry.total += Number(order.total_amount) || 0;
      orderStats.set(order.user_id, entry);
    });

    const enhancedUsers: User[] = (usersData as User[] | null || []).map((user) => {
      const vendorInfo = vendorMap.get(user.id);
      const orderInfo = orderStats.get(user.id);
      return {
        ...user,
        vendorBusiness: vendorInfo?.business_name || null,
        vendorVerified: vendorInfo?.is_verified ?? null,
        orders_count: orderInfo?.count ?? 0,
        total_spent: orderInfo?.total ?? 0,
      };
    });
            ...
            onClick={async () => {
              if (filteredUsers.length === 0) {
                if (typeof window !== "undefined") {
                  window.alert("No users available to export right now.");
                }
                return;
              }
              setExporting(true);
