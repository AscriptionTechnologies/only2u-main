...
export default function TaxSummaryReport() {
  ...
  const [groupBy, setGroupBy] = useState<"month" | "type" | "supply">("month");
  ...
  const reportData = useMemo(() => {
    ...
    invoices.forEach((invoice) => {
      ...
      } else if (groupBy === "supply") {
        key = invoice.supply_type;
      }
      ...
      grouped[key].totalTax +=
        parseFloat(summary.totalCGST || 0) +
        parseFloat(summary.totalSGST || 0) +
        parseFloat(summary.totalIGST || 0);
    });
  ...
  const totals = useMemo(() => {
    return reportData.reduce(
      ...
      { count: 0, taxableValue: 0, cgst: 0, sgst: 0, igst: 0, totalTax: 0 }
    );
  }, [reportData]);

  const handleExport = () => {
    const csv = [
      ["Period/Type/Supply", "Invoice Count", "Taxable Value", "CGST", "SGST", "IGST", "Total Tax"],
      ...reportData.map((item) => [
        item.key,
        item.count,
    ...
    a.href = url;
    a.download = `tax-summary-${dateRange.start}-to-${dateRange.end}.csv`;
    a.click();
  };
  ...
  return (
    <div className="p-6">
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 mb-2">Tax Summary Report</h2>
          <p className="text-gray-600">GST liability and tax breakdown analysis</p>
        </div>
        <button
      ...
      <div className="mb-6 grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div>
          ...
          >
            ...
            <option value="supply">Supply Type</option>
          </select>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-6">
        ...
        <div className="bg-green-50 border border-green-200 rounded-lg p-4">
          <div className="text-sm text-green-600 font-medium mb-1">CGST</div>
          <div className="text-2xl font-bold text-green-900">
            ₹{totals.cgst.toLocaleString("en-IN", { minimumFractionDigits: 2 })}
          </div>
        </div>
        <div className="bg-orange-50 border border-orange-200 rounded-lg p-4">
          <div className="text-sm text-orange-600 font-medium mb-1">SGST</div>
          <div className="text-2xl font-bold text-orange-900">
            ₹{totals.sgst.toLocaleString("en-IN", { minimumFractionDigits: 2 })}
          </div>
        </div>
        <div className="bg-red-50 border border-red-200 rounded-lg p-4">
          <div className="text-sm text-red-600 font-medium mb-1">IGST</div>
            ...
            <thead className="bg-gray-50">
              <tr>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                  {groupBy === "month" ? "Month" : groupBy === "type" ? "Invoice Type" : "Supply Type"}
                </th>
              ...
              {reportData.map((item, index) => (
                <tr key={index} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-semibold text-gray-900 text-right">
                    ₹{item.totalTax.toLocaleString("en-IN", { minimumFractionDigits: 2 })}
                  </td>
                </tr>
              ))}
