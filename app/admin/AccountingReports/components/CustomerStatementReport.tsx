...
export default function CustomerStatementReport() {
  ...
  const fetchCustomers = async () => {
    try {
      ...
      const uniqueCustomers = Array.from(
        new Set((data || []).map((inv: any) => inv.buyer?.name).filter(Boolean))
      ) as string[];
  ...
  const fetchCustomerInvoices = async () => {
    ...
    try {
      ...
      const filtered = (data || []).filter(
        (inv: any) => inv.buyer?.name === selectedCustomer
      ) as Invoice[];
  ...
  const customerInfo = invoices[0]?.buyer || {};

  const handleExport = () => {
    const csv = [
      ["Customer Statement", "", ""],
      ["Customer Name", customerInfo.name || selectedCustomer, ""],
    ...
    a.href = url;
    a.download = `customer-statement-${selectedCustomer}-${dateRange.start}-to-${dateRange.end}.csv`;
    a.click();
  };
    ...
    <div className="p-6">
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600">Individual customer account statement</p>
        </div>
        {selectedCustomer && (
          <button
      ...
      {!selectedCustomer ? (
        <div className="text-center py-12 bg-gray-50 rounded-lg">
          <p className="text-gray-600">Please select a customer to view their statement</p>
        </div>
      ) : loading ? (
        ...
        <div className="space-y-6">
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
              <div>
                <span className="text-gray-600">Name:</span> <span className="font-medium">{customerInfo.name}</span>
              </div>
              {customerInfo.gstin && (
                <div>
                  <span className="text-gray-600">GSTIN:</span>{" "}
                  <span className="font-mono">{customerInfo.gstin}</span>
                </div>
              )}
              {customerInfo.address && (
                <div className="col-span-2">
                  <span className="text-gray-600">Address:</span> <span>{customerInfo.address}</span>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-green-50 border border-green-200 rounded-lg p-4">
              <div className="text-sm text-green-600 font-medium mb-1">Total Invoices</div>
