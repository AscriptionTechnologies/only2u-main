...
import { exportColors } from "./colorExportUtils";
...
const ColorManagementPage = () => {
  ...
  const fetchColors = async () => {
    setLoading(true);
    const [{ data: colorData, error: colorError }, { data: usageData, error: usageError }] =
      await Promise.all([
    ...
    if (usageError) {
      console.error("Error fetching color usage:", usageError);
    }

    const usageCounts = new Map<string, number>();
    (usageData || []).forEach((variant: any) => {
      const colorId = variant.color_id;
      if (!colorId) return;
      usageCounts.set(colorId, (usageCounts.get(colorId) || 0) + 1);
    });

    const enrichedColors = (colorData || []).map((color: any) => ({
      ...color,
      usage_count: usageCounts.get(color.id) || 0,
    }));
          ...
          onClick={async () => {
            if (colors.length === 0) {
              if (typeof window !== "undefined") {
                window.alert("No colors available to export right now.");
              }
              return;
            }
            setExporting(true);
            try {
              ...
              if (typeof window !== "undefined") {
                window.alert("Failed to export colors. Please try again.");
              }
            } finally {
              setExporting(false);
            }
          }}
          disabled={exporting || colors.length === 0}
          className={`flex items-center gap-2 px-4 py-2 rounded-lg border text-sm font-medium transition-colors ${
            exporting || colors.length === 0
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <div className="text-xs text-gray-500 mt-1">
                    Usage: {color.usage_count ?? 0} variant{(color.usage_count ?? 0) === 1 ? "" : "s"}
                  </div>
                </div>
              </div>
