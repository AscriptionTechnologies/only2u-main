...
export async function POST(request: NextRequest) {
  try {
    const { question_id, vendor_id, answer_text } = await request.json();

    if (!question_id || !vendor_id || !answer_text) {
      return NextResponse.json(
        { error: 'Missing required fields: question_id, vendor_id, and answer_text' },
        { status: 400 }
      );
    }
